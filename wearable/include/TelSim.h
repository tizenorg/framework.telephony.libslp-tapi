...
/**
 * @struct TelSimInitInfo_t
 * This struct has informations that card_status and whether it is changed.
 */
typedef struct {
	TelSimCardStatus_t card_status;
	unsigned char b_is_changed;
} TelSimInitInfo_t;
...
/**
 * This union data contains RESPONSE data types for all supported operations of tel_call_sim_sync().
 */
typedef union {
	TelSimInitInfo_t init_info;
	TelSimCardType_t card_type;
	TelSimImsiInfo_t imsi_info;
	TelSimEccList_t ecc_list;
	TelSimIccIdInfo_t iccid_info;
	TelSimCphsNetName_t cphs_netname;
	TelSimSpn_t spn;
	TelSimMsisdnList_t msisdn_list;
	TelSimApduResp_t apdu_resp;
	TelSimAtrResp_t atr_resp;
}TelSimResponseData_t;

/**
 * This union data contains REQUEST data types for all supported operations of tel_call_sim_sync().
 */
typedef union {
	TelSimApdu_t apdu;
}TelSimRequestData_t;
