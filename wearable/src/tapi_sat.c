...
static GVariant* _tapi_sat_gen_app_exec_result_data(TelSatAppsRetInfo_t *result_info)
{
	...
	switch(result_info->commandType)
	{
		...
		case TAPI_SAT_CMD_TYPE_SEND_SS:{
			...
			for (index = 0; index < ss_str_len; index++) {
				dbg("index(%d) data(%d)", index, result_info->appsRet.sendSs.ssString.string[index]);
				g_variant_builder_add(builder, "y", result_info->appsRet.sendSs.ssString.string[index]);
			}
	...
	if(items && item_cnt > 0){
		...
		while(g_variant_iter_loop(iter,"(si)",&item_str, &item_id)){
			...
			if(strlen(item_str) > TAPI_SAT_DEF_ITEM_STR_LEN_MAX + 6){
				...
				memcpy(pMainMenu->satMainMenuItem[index].itemString, item_str, strlen(item_str));
			}
