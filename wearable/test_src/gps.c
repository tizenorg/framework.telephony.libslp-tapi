...
static char data_smart_assistant_area_index[MENU_DATA_SIZE + 1] = "1";
static char data_smart_assistant_area_mode[MENU_DATA_SIZE + 1] = "1";
...
static void on_noti_area_status(TapiHandle *handle, const char *noti_id, void *data, void *user_data)
{
	struct tel_noti_smart_assistant_area_status *noti = (struct tel_noti_smart_assistant_area_status *)data;

	msg("");
	msgb("event(%s) receive !!", TAPI_NOTI_SMART_ASSISTANT_AREA_STATUS);
	msg("data=%p", data);
	if(!data)
		return;

	msg(" - area_status(1:IN, 2:OUT) = %d", noti->area_status);
	msg(" - index = %d", noti->index);
}

static void on_noti_sync_status(TapiHandle *handle, const char *noti_id, void *data, void *user_data)
{
	struct tel_noti_smart_assistant_sync_status *noti = (struct tel_noti_smart_assistant_sync_status *)data;

	msg("");
	msgb("event(%s) receive !!", TAPI_NOTI_SMART_ASSISTANT_SYNC_STATUS);
	msg("data=%p", data);
	if(!data)
		return;

	msg(" - init_status(1:Success, 2:Fail) = %d", noti->init_status);
	msg(" - init_fail_cause = %d", noti->init_fail_cause);
}
...
static void on_enable_smart_assistant(TapiHandle *handle, int result, void *data, void *user_data)
{
	msg("");
	msgb("tel_enable_smart_assistant() response receive");
	msg(" - result = 0x%x", result);
}

static void on_disable_smart_assistant(TapiHandle *handle, int result, void *data, void *user_data)
{
	msg("");
	msgb("tel_disable_smart_assistant() response receive");
	msg(" - result = 0x%x", result);
}

static void on_sync_smart_assistant_area_list(TapiHandle *handle, int result, void *data, void *user_data)
{
	msg("");
	msgb("tel_sync_smart_assistant_area_list() response receive");
	msg(" - result = 0x%x", result);
}

static void on_del_smart_assistant_area_list(TapiHandle *handle, int result, void *data, void *user_data)
{
	msg("");
	msgb("tel_del_smart_assistant_area_list() response receive");
	msg(" - result = 0x%x", result);
}

static void on_add_smart_assistant_area(TapiHandle *handle, int result, void *data, void *user_data)
{
	msg("");
	msgb("tel_add_smart_assistant_area() response receive");
	msg(" - result = 0x%x", result);
}

static void on_modify_smart_assistant_area(TapiHandle *handle, int result, void *data, void *user_data)
{
	msg("");
	msgb("tel_set_gps_frequency_aiding() response receive");
	msg(" - result = 0x%x", result);
...
static void on_set_smart_assistant_info(TapiHandle *handle, int result, void *data, void *user_data)
{
	msg("");
	msgb("tel_set_smart_assistant_info() response receive");
	msg(" - result = 0x%x", result);
}
...
static int run_enable_smart_assistant(MManager *mm, struct menu_data *menu)
{
	TapiHandle *handle = menu_manager_ref_user_data(mm);
	int result;
	msg("call tel_enable_smart_assistant()");

	result = tel_enable_smart_assistant(handle, on_enable_smart_assistant, NULL);
	if (result != TAPI_API_SUCCESS) {
		msg("failed. (result = %d)", result);
	}
	msg("exit");
...
static int run_disable_smart_assistant(MManager *mm, struct menu_data *menu)
{
	TapiHandle *handle = menu_manager_ref_user_data(mm);
	int result;
	msg("call tel_disable_smart_assistant()");

	result = tel_disable_smart_assistant(handle, on_disable_smart_assistant, NULL);
	if (result != TAPI_API_SUCCESS) {
		msg("failed. (result = %d)", result);
	}
	msg("exit");
...
static int run_sync_smart_assistant_area_list(MManager *mm, struct menu_data *menu)
{
	TapiHandle *handle = menu_manager_ref_user_data(mm);
	...
	area_list.area[2].index = 3;
	area_list.area[2].mode_state = TAPI_SMART_ASSISTANT_MODE_STATE_START;
	msg("call tel_sync_smart_assistant_area_list()");

	result = tel_sync_smart_assistant_area_list(handle, &area_list, on_sync_smart_assistant_area_list, NULL);
	if (result != TAPI_API_SUCCESS) {
		msg("failed. (result = %d)", result);
	}
	msg("exit");
...
static int run_del_smart_assistant_area_list(MManager *mm, struct menu_data *menu)
{
	TapiHandle *handle = menu_manager_ref_user_data(mm);
	TelSmartAssistantAreaList_t area_list;
	int result;
	...
	msg("call tel_del_smart_assistant_area_list()");

	result = tel_del_smart_assistant_area_list(handle, &area_list, on_del_smart_assistant_area_list, NULL);
	if (result != TAPI_API_SUCCESS) {
		msg("failed. (result = %d)", result);
	}
	msg("exit");
...
static int run_add_smart_assistant_area(MManager *mm, struct menu_data *menu)
{
	TapiHandle *handle = menu_manager_ref_user_data(mm);
	TelSmartAssistantArea_t area;
	int result;

	area.index = atoi(data_smart_assistant_area_index);
	area.mode_state = atoi(data_smart_assistant_area_mode);

	msg("call tel_add_smart_assistant_area() index=%d", area.index);

	result = tel_add_smart_assistant_area(handle, &area, on_add_smart_assistant_area, NULL);
	if (result != TAPI_API_SUCCESS) {
		msg("failed. (result = %d)", result);
	}
	msg("exit");
...
static int run_modify_smart_assistant_area(MManager *mm, struct menu_data *menu)
{
	TapiHandle *handle = menu_manager_ref_user_data(mm);
	TelSmartAssistantArea_t area;
	int result;
	...
	msg("call tel_modify_smart_assistant_area() index=%d", area.index);

	result = tel_modify_smart_assistant_area(handle, &area, on_modify_smart_assistant_area, NULL);
	if (result != TAPI_API_SUCCESS) {
		msg("failed. (result = %d)", result);
	}
	msg("exit");
...
static int run_set_smart_assistant_info(MManager *mm, struct menu_data *menu)
{
	TapiHandle *handle = menu_manager_ref_user_data(mm);
	TelSmartAssistantInfo_t info;
	int result;

	info.index = atoi(data_smart_assistant_area_index);
	info.lpp_state = atoi(data_smart_assistant_area_mode);

	msg("call tel_set_smart_assistant_info() index=%d", info.index);

	result = tel_set_smart_assistant_info(handle, &info, on_set_smart_assistant_info, NULL);
	if (result != TAPI_API_SUCCESS) {
		msg("failed. (result = %d)", result);
	}
	msg("exit");
...
static struct menu_data menu_enable_smart_assistant[] = {
	{ "1", "run", NULL, run_enable_smart_assistant, NULL},
	{ NULL, NULL, },
};

static struct menu_data menu_disable_smart_assistant[] = {
	{ "1", "run", NULL, run_disable_smart_assistant, NULL},
	{ NULL, NULL, },
};

static struct menu_data menu_sync_smart_assistant_area_list[] = {
	{ "1", "run", NULL, run_sync_smart_assistant_area_list, NULL},
	{ NULL, NULL, },
};

static struct menu_data menu_del_smart_assistant_area_list[] = {
	{ "1", "run", NULL, run_del_smart_assistant_area_list, NULL},
	{ NULL, NULL, },
};

static struct menu_data menu_add_smart_assistant_area[] = {
	{ "1", "index (1=Default Index)", NULL, NULL, data_smart_assistant_area_index},
	{ "2", "mode (1=Enable, 2=Disable)", NULL, NULL, data_smart_assistant_area_mode},
	{ "3", "run", NULL, run_add_smart_assistant_area, NULL},
	{ NULL, NULL, },
};

static struct menu_data menu_modify_smart_assistant_area[] = {
	{ "1", "index (1=Default Index)", NULL, NULL, data_smart_assistant_area_index},
	{ "2", "mode (1=Enable, 2=Disable)", NULL, NULL, data_smart_assistant_area_mode},
	{ "3", "run", NULL, run_modify_smart_assistant_area, NULL},
	{ NULL, NULL, },
};

static struct menu_data menu_set_smart_assistant_info[] = {
	{ "1", "index (1=Default Index)", NULL, NULL, data_smart_assistant_area_index},
	{ "2", "lpp_state (1=Start, 2=Stop)", NULL, NULL, data_smart_assistant_area_mode},
	{ "3", "run", NULL, run_set_smart_assistant_info, NULL},
	{ NULL, NULL, },
};
...
struct menu_data menu_gps[] = {
	...
	{ "3", "ENABLE Smart Assistant", menu_enable_smart_assistant, NULL, NULL},
	{ "4", "DISABLE Smart Assistant", menu_disable_smart_assistant, NULL, NULL},
	{ "5", "SYNC Smart Assistant Area List", menu_sync_smart_assistant_area_list, NULL, NULL},
	{ "6", "DELETE Smart Assistant Area List", menu_del_smart_assistant_area_list, NULL, NULL},
	{ "7", "ADD Smart Assistant Area", menu_add_smart_assistant_area, NULL, NULL},
	{ "8", "MODIFY Smart Assistant Area", menu_modify_smart_assistant_area, NULL, NULL},
	{ "9", "SET Smart Assistant info", menu_set_smart_assistant_info, NULL, NULL},
	{ NULL, NULL, },
};
...
void register_gps_event(TapiHandle *handle)
{
	...
	ret = tel_register_noti_event(handle, TAPI_NOTI_SMART_ASSISTANT_AREA_STATUS, on_noti_area_status, NULL);
	if (ret != TAPI_API_SUCCESS) {
		msg("event register failed(%d)", ret);
	}

	ret = tel_register_noti_event(handle, TAPI_NOTI_SMART_ASSISTANT_SYNC_STATUS, on_noti_sync_status, NULL);
	if (ret != TAPI_API_SUCCESS) {
		msg("event register failed(%d)", ret);
	}
}
