...
static char data_sim_op_type[MENU_DATA_SIZE + 1] = "1";
...
static int run_sim_request_sync (MManager *mm, struct menu_data *menu)
{
	TapiHandle *handle = menu_manager_ref_user_data (mm);
	int rt = 0;
	TelSimResponseData_t outparam;
	TelSimRequestData_t inparam;
	int i = 0;
	unsigned char op = 0;

	memset (&outparam, 0, sizeof(TelSimResponseData_t));
	memset (&inparam, 0, sizeof(TelSimRequestData_t));

	msg("call [%s] tapi service !!!", menu->title);

	if (strlen(data_sim_op_type) == 1) {
		op = data_sim_op_type[0] - '0';
	} else {
		op = ((data_sim_op_type[0] - '0') * 10) + (data_sim_op_type[1] - '0');
	}
	msg("op = [%d]\n", op);

	switch(op) {
	case TAPI_SIM_GET_IMSI:
		rt = tel_request_sim_sync(handle, op, NULL, &outparam);
		msg("mcc[%s]", outparam.imsi_info.szMcc);
		msg("mnc[%s]", outparam.imsi_info.szMnc);
		msg("msin[%s]", outparam.imsi_info.szMsin);
		break;
	case TAPI_SIM_GET_ICCID:
		rt = tel_request_sim_sync(handle, op, NULL, &outparam);
		msg("iccid length[%d]", outparam.iccid_info.icc_length);
		msg("iccid[%s]", outparam.iccid_info.icc_num);
		break;
	case TAPI_SIM_GET_INIT_STATUS:
		rt = tel_request_sim_sync(handle, op, NULL, &outparam);
		msg("status[%d]", outparam.init_info.card_status);
		msg("new_sim[%d]", outparam.init_info.b_is_changed);
		break;
	case TAPI_SIM_GET_MSISDN:
		rt = tel_request_sim_sync(handle, op, NULL, &outparam);
		msg("list->count[%d]", outparam.msisdn_list.count);
		for (i = 0; i < outparam.msisdn_list.count; i++)
			msg("index[%d] - name[%s], num[%s]", i, outparam.msisdn_list.list[i].name,
					outparam.msisdn_list.list[i].num);
		break;
	case TAPI_SIM_GET_SPN:
		rt = tel_request_sim_sync(handle, op, NULL, &outparam);
		msg("spn name[%s]", outparam.spn.spn);
		msg("spn condition[0x%x]", outparam.spn.display_condition);
		break;
	case TAPI_SIM_GET_CPHS_NET_NAME:
		rt = tel_request_sim_sync(handle, op, NULL, &outparam);
		msg("cphs full name[%s]", outparam.cphs_netname.full_name);
		msg("cphs short name[%s]", outparam.cphs_netname.short_name);
		break;
	case TAPI_SIM_TRANSFER_APDU: {
		...
		inparam.apdu.apdu_len = len / 2;
		inparam.apdu.apdu = (unsigned char*) calloc (inparam.apdu.apdu_len, 1);
		if(inparam.apdu.apdu != NULL) {
			memcpy (inparam.apdu.apdu, result_apdu, inparam.apdu.apdu_len);
			rt = tel_request_sim_sync(handle, op, &inparam, &outparam);
	...
	case TAPI_SIM_GET_ATR:
		rt = tel_request_sim_sync(handle, op, NULL, &outparam);
		msg("r_atr->atr_resp_len[%d]", outparam.atr_resp.atr_resp_len);
		if (outparam.atr_resp.atr_resp_len) {
			for (i = 0; i < outparam.atr_resp.atr_resp_len; i++)
				msg("[%d]`s byte is [0x%x]", i, outparam.atr_resp.atr_resp[i]);
		}
		break;
...
static void on_sim_request_async (TapiHandle *handle, int result, void *data,
		void *user_data)
{
	TelSimAccessResult_t access_rt = result;
	TelSimResponseData_t *resp = data;
	TelSimTapiOperation_t *op = user_data;
	int i = 0;

	msg("tel_sim_request_async() response received");
	msg("access_rt[%d]", access_rt);
	CHECK_ACCESS_RT(access_rt);
	...
	switch(*op) {
	case TAPI_SIM_GET_INIT_STATUS:
		msg("status[%d]", resp->init_info.card_status);
		msg("new_sim[%d]", resp->init_info.b_is_changed);
		break;
	case TAPI_SIM_GET_IMSI:
		msg("mcc[%s]", resp->imsi_info.szMcc);
		msg("mnc[%s]", resp->imsi_info.szMnc);
		msg("msin[%s]", resp->imsi_info.szMsin);
		break;
	case TAPI_SIM_GET_ICCID:
		msg("iccid length[%d]", resp->iccid_info.icc_length);
		msg("iccid[%s]", resp->iccid_info.icc_num);
		break;
	case TAPI_SIM_GET_MSISDN:
		msg("list->count[%d]", resp->msisdn_list.count);
		for (i = 0; i < resp->msisdn_list.count; i++)
			msg("index[%d] - name[%s], num[%s]", i, resp->msisdn_list.list[i].name,
					resp->msisdn_list.list[i].num);
		break;
	case TAPI_SIM_GET_SPN:
		msg("spn name[%s]", resp->spn.spn);
		msg("spn condition[0x%x]", resp->spn.display_condition);
		break;
	case TAPI_SIM_GET_CPHS_NET_NAME:
		msg("cphs full name[%s]", resp->cphs_netname.full_name);
		msg("cphs short name[%s]", resp->cphs_netname.short_name);
		break;
	case TAPI_SIM_TRANSFER_APDU: {
		msg("outparam.apdu_resp.apdu_resp_len[%d]", resp->apdu_resp.apdu_resp_len);
	...
	case TAPI_SIM_GET_ATR:
		msg("atr_resp_len[%d]", resp->atr_resp.atr_resp_len);
		if (resp->atr_resp.atr_resp_len) {
			for (i = 0; i < resp->atr_resp.atr_resp_len; i++)
				msg("[%d]`s byte is [0x%x]", i, resp->atr_resp.atr_resp[i]);
		}
		break;
...
static int run_sim_request_async (MManager *mm, struct menu_data *menu)
{
	TapiHandle *handle = menu_manager_ref_user_data (mm);
	int rt = 0;
	TelSimResponseData_t outparam;
	...
	case TAPI_SIM_GET_CPHS_NET_NAME:
	case TAPI_SIM_GET_ATR:
		rt = tel_request_sim_async(handle, *op, NULL, on_sim_request_async, op);
		break;
	case TAPI_SIM_TRANSFER_APDU: {
		unsigned char result_apdu[MENU_DATA_SIZE + 1] = { 0, };
		int i = 0, j = 0, len = 0, hex = 0;
		...
		if(inparam.apdu.apdu != NULL) {
			memcpy (inparam.apdu.apdu, result_apdu, inparam.apdu.apdu_len);
			rt = tel_request_sim_async(handle, *op, &inparam, on_sim_request_async, op);
			free(inparam.apdu.apdu);
		} else {
			msg("Memory allocation failed");
		}
...
static struct menu_data menu_sim_call_sync[] = {
	{ "1", "op_type (1:get_init_status, 3:get_imsi, 5:get_iccid,\n\t\t"
			"16:get_msisdn, 18:get_spn, 19:get_cphs_net_name, 28:transfer_apdu,\n\t\t"
			"29:get_atr)", NULL, NULL, data_sim_op_type},
	{ "2", "run", NULL, run_sim_request_sync, NULL},
	{ NULL, NULL, },
};

static struct menu_data menu_sim_call_async[] = {
	{ "1", "op_type (1:get_init_status, 3:get_imsi, 5:get_iccid,\n\t\t"
			"16:get_msisdn, 18:get_spn, 19:get_cphs_net_name, 28:transfer_apdu,\n\t\t"
			"29:get_atr)", NULL, NULL, data_sim_op_type},
	{ "2", "run", NULL, run_sim_request_async, NULL},
	{ NULL, NULL, },
};
...
struct menu_data menu_sim[] = {
	...
	{"50", "tel_call_sim_sync", menu_sim_call_sync, NULL, NULL},
	{"51", "tel_call_sim_async", menu_sim_call_async, NULL, NULL},
	{ NULL, NULL, },
};
